...
if st.button("Connect/Disconnect API"):
    if api_key:
        ...
        if selected_api == "Gemini API":
            ...
            st.write(f"Connecting to Gemini API with key: {api_key[:5]}... (Partially shown for security)")
            # Display first few characters for confirmation
            # ... your Gemini API interaction code ...
            status.text("Status: Connected to Gemini API")
            # Update the status
        elif selected_api == "Our AI API":
            # Add code here to connect/disconnect Our AI API using the api_key and our_ai_chat
            st.write(f"Connecting to Our AI API with key: {api_key[:5]}... (Partially shown for security)")
            # Display first few characters for confirmation
            # ... your Our AI API interaction code ...
            status.text("Status: Connected to Our AI API")
            # Update the status
    else:
